...
const Profile = () => {
  ...
  const { firstName, lastName, image } = formValue;
  const navigate = useNavigate();
  // const [firstName, setFirstName] = useState("");
  // const [lastName, setLastName] = useState("");
  // const [image, setImage] = useState(null);
  const [hovered, setHovered] = useState(false);
  const [selectedColor, setSelectdColor] = useState(0);
  const fileInputRef = useRef(null);

  useEffect(() => {
    if (userInfo.profileSetup) {
      setSelectdColor(userInfo.color);
      setFormValue({ ...userInfo });
    }
    if (userInfo.image) {
      setFormValue({ ...userInfo, image: `${HOST}/${userInfo.image}` });
  ...
  const handleChangeImage = async (e) => {
    ...
    if (file) {
      ...
      if (res.status === 200 && res.data.image) {
        ...
        toast.success("Image updated seccessfully");
      }
          ...
          >
            ...
            {hovered && (
              <div
                ...
                onClick={image ? handleDeleteImage : handleClickFileInput}
              >
                {image ? (
